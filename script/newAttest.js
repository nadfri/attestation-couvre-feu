...
let listAttest_Storage =
	JSON.parse(localStorage.getItem("listAttest_Storage")) != null
...
let heureNow = Intl.DateTimeFormat("fr-FR", {
	hour: "numeric",
	minute: "numeric",
}).format(Date.now());
heureSortie.value = "19:00" > heureNow && heureNow > "06:00" ? "19:00" : heureNow;
...
form.onsubmit = (e) => {
	...
	if (count_users == 0) {
		...
		let dateCreation = Intl.DateTimeFormat("fr-FR", optionDate).format(
			Date.now()
		);
		let heureCreation = Intl.DateTimeFormat("fr-FR", {
			hour: "numeric",
			minute: "numeric",
		}).format(Date.now());
		...
		let objet_final = {
			...
			dateSortie: dateSortie.value.replace(
				/(\d{4})-(\d{2})-(\d{2})/,
				"$3/$2/$1"
			), // jj/mm/aaaa
		...
		localStorage.setItem(
			"listAttest_Storage",
			JSON.stringify(listAttest_Storage)
		);
